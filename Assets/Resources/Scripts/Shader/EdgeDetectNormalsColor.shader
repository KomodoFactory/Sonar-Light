...
//colours not in here?
		...
		float3 tempPos = mul(_Object2World, o.pos).xyz;	

		float dx = _Position[0] - tempPos.x;
		float dy = _Position[1] - tempPos.y;
		float dz = _Position[2] - tempPos.z;

		o.distance = sqrt(dx*dx + dy*dy + dz*dz);
		...
		float4 fragD(v2fd i) : SV_Target
	{
		...
		half4 fragRobert(v2f i) : SV_Target{
			...
			if (edge > 0)
				return  i.pos/i.distance*_Distance;//lerp(tex2D(_MainTex, i.uv[0].xy), _BgColor, _BgFade); //return 0 to have edge only mode
			 else
